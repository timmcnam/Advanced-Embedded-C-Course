...
volatile static unsigned int motorCount = 0;
...
{
  if((iAmSpeed == 1) && (motorCount < 9)) // 90% duty Cycle
   {
      PTP = 0x01;  // Turns the motor.  
   }
   if((slowSpeed == 1) && (motorCount < 3)) // 30% duty Cycle
   {
     PTP = 0x01;   // Turns the motor.
   }
   else
...
void ThePoshTaskScheduler(void)
{
   ...
   if(moto > 0)
   {
      moto--;         // reset the incremented value to 0
      MotorControl(); // Call the motor operation function
      motorCount++;   // increment motorCount at a rate of 2ms
   }
   if(check > 0)
   {
...
void interrupt TimerInterruptGuy(void)
{
   ...
   if((twoMilliCount % 10) == 0)
   {
      moto++; // increment this value every 2ms
   ...
   if(twoMilliCount > 10)
   {
      twoMilliCount = 0; // resets this value every 2ms 
   }
